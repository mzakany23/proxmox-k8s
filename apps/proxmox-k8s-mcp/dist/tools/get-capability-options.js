 ...
 */
import { z } from "zod";
import { getPods } from "../clients/kubernetes.js";
...
export async function getCapabilityOptions(params) {
    ...
    for (const term of definition.searchTerms) {
        // Check pods
        const matchingPods = pods.filter((p) => p.name.toLowerCase().includes(term) ||
        ...
        // Check registry
        const matchingApps = registry.apps.filter((a) => a.name.toLowerCase().includes(term) ||
    ...
    if (existingResources.length > 0) {
        response.considerations.unshift(`Found ${existingResources.length} existing ${capability} resource(s) that may be reusable`);
    }
    return {
        content: [{
                type: "text",
...
//# sourceMappingURL=get-capability-options.js.map