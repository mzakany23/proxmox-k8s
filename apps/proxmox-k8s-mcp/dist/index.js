...
// ============================================================================
server.tool("list_services", "List all deployed services in the Kubernetes cluster. Returns service names, namespaces, and status.", listServicesSchema, listServices);
server.tool("get_service_info", "Get detailed information about a specific service including pods, endpoints, ingress, and ArgoCD status.", getServiceInfoSchema, getServiceInfo);
server.tool("check_health", "Check the health status of a deployed service. Runs 7 checks: pods, endpoints, ingress, certificate, ArgoCD, DNS, HTTP.", checkHealthSchema, checkHealth);
server.tool("get_capability_options", "Get options for adding a capability (database, cache, queue, storage, etc.). Returns existing resources, new options, and questions to consider.", getCapabilityOptionsSchema, getCapabilityOptions);
server.tool("get_deployment_pattern", "Get the recommended deployment pattern and template for a given application type (e.g., 'python-api', 'frontend', 'database').", getDeploymentPatternSchema, getDeploymentPattern);
server.tool("get_deployment_steps", "Get the detailed steps needed to deploy an application. Returns prerequisites, ordered steps with commands, and rollback instructions.", getDeploymentStepsSchema, getDeploymentSteps);
// ============================================================================
// EXECUTION TOOLS - For executor subagent only, after plan approval
// ============================================================================
server.tool("apply_manifest", "Apply a Kubernetes manifest. Minimal execution tool for use after a deployment plan has been approved. Supports dry-run mode.", applyManifestSchema, applyManifest);
// ============================================================================
// RESOURCES - Static/dynamic information sources
// ============================================================================
server.resource("services://inventory", "Current inventory of all deployed services with live status from cluster", async () => {
...
server.resource("templates://catalog", "Available deployment templates and their descriptions", async () => {
...
server.resource("architecture://overview", "Architecture overview extracted from CLAUDE.md", async () => {
