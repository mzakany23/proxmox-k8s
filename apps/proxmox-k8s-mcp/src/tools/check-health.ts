...
export async function checkHealth(params: {
  ...
  // 1. Pod status
  try {
    const pods = getPods(namespace).filter(
  ...
  try {
    ...
    if (resourceExists("certificate", certName, namespace)) {
      ...
      const ready = cert.status?.conditions?.find((c: any) => c.type === "Ready")?.status === "True";
  ...
  try {
    const httpCode = execSync(
      `curl -sk -o /dev/null -w "%{http_code}" "${targetUrl}" --connect-timeout 5`,
      { encoding: "utf-8" }
    ).trim();
