...
def create_mcp_server(host: str = "127.0.0.1", port: int = 8000) -> FastMCP:
    ...
    """
    mcp = FastMCP(name="monarch-money", host=host, port=port)

    # Session factory for tools/resources
    @asynccontextmanager
    ...
    register_account_tools(mcp, get_session)
    register_transaction_tools(mcp, get_session)
    register_analytics_tools(mcp, get_session)
    register_sync_tools(mcp, get_session)
    register_export_tools(mcp, get_session)

    # Register resources
    from .resources import register_resources

    register_resources(mcp, get_session)
