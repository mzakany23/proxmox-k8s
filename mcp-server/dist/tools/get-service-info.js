...
import { kubectl, getPods, getServices, getIngresses, resourceExists, } from "../clients/kubernetes.js";
...
export async function getServiceInfo(params) {
    ...
    info.pods = allPods.filter((p) => p.name.startsWith(name) || p.name.includes(name));
    // Get services
    const allServices = getServices(namespace);
    info.services = allServices.filter((s) => s.name === name || s.name.startsWith(name));
    // Get ingress
    ...
    try {
        ...
        if (resourceExists("certificate", certName, namespace)) {
            ...
            info.certificate = {
                name: certName,
                ready: cert.status?.conditions?.find((c) => c.type === "Ready")?.status === "True",
    ...
    try {
        ...
        if (ep.subsets) {
            info.endpoints = ep.subsets.flatMap((s) => (s.addresses || []).map((a) => a.ip));
        }
    }
    catch {
        // No endpoints
    ...
    // Summary
    info.summary = {
        healthy: info.pods.every((p) => p.status === "Running") && info.pods.length > 0,
...
//# sourceMappingURL=get-service-info.js.map